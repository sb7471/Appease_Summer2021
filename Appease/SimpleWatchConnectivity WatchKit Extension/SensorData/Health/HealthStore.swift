...
//    var fileHandler = FileIOManager(directory: URL(fileURLWithPath: NSTemporaryDirectory(), isDirectory: true), fileName: ProcessInfo().globallyUniqueString)
    ...
    init() {
        ...
        timerInvoke()
        NotificationCenter.default.addObserver(forName:Notification.Name.loginFromIphoneToken, object:nil, queue:nil,using:receiveToken)
    }
    
    
    func receiveToken(notification: Notification) {
        if let userInfo = notification.userInfo, let info = userInfo["loginFromIphoneToken"] {
            self.userToken = (info as AnyObject).description
            print("inside receiveToken")
            print(self.userToken)
       }
    }
    
    @objc func handleMessage(notification: NSNotification) {
        if let dict = notification.object as? NSDictionary {
            if let myMessage = dict["myMessage"] as? String{
                self.userToken = myMessage
            }
        }
    }
...
//                    sampleCount += 1
    ...
    func writeDataToFile(timeStamp: Date) {
        DispatchQueue.main.async {
            ...
            var data: Dictionary<String, String> = [
                ...
                "userToken": String(self.userToken),
    ...
    @objc func sendFile(){
		...
		WCSession.default.sendMessage(["healthMessageFromWatch" : self.healthMessage], replyHandler:nil)
	#else
        WCSession.default.transferFile(file: fileHandler.fullURL, metadata: fileMetaData)
	#endif
    }
